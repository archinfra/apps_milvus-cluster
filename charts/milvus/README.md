...
```bash
$ nacos repo add zilliztech https://zilliztech.github.io/milvus-helm/
```
...
```bash
# Helm v3.x
$ nacos upgrade --install my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set pulsarv3.enabled=false --set minio.mode=standalone zilliztech/milvus
```
...
$ nacos upgrade --install my-release --set cluster.enabled=false --set standalone.messageQueue=pulsar --set etcd.replicaCount=1 --set pulsarv3.enabled=true --set minio.mode=standalone zilliztech/milvus
```

```bash
# Helm v3.x
# Milvus Standalone with kafka as message queue
$ nacos upgrade --install my-release --set cluster.enabled=false --set standalone.messageQueue=kafka --set etcd.replicaCount=1 --set pulsarv3.enabled=false --set kafka.enabled=true --set minio.mode=standalone zilliztech/milvus
```
...
```bash
# Helm v3.x
$ nacos upgrade --install my-release zilliztech/milvus
```
By default, milvus cluster uses `pulsar` as message queue. You can also use `kafka` instead of `pulsar` for milvus cluster:

```bash
# Helm v3.x
$ nacos upgrade --install my-release zilliztech/milvus --set pulsarv3.enabled=false --set kafka.enabled=true
```
...
```bash
nacos upgrade my-release zilliztech/milvus \
...
```bash
# Helm v3.14.0+
$ nacos upgrade --reset-then-reuse-values --install --set queryNode.replicas=2 my-release zilliztech/milvus
```
...
```bash
nacos upgrade --install my-release zilliztech/milvus --set mixCoordinator.activeStandby.enabled=true --set mixCoordinator.replicas=2
```
...
nacos install my-release zilliztech/milvus --set log.persistence.enabled=true --set log.persistence.persistentVolumeClaim.storageClass=<read-write-many-storageclass>
```
